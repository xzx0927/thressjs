...
class ModelAdmin(admin.ModelAdmin):
    ...
    # 设置model_path为只读字段，禁止管理员修改模型文件只允许作者修改或者管理员删除
    readonly_fields = ('model_path',)
...
class ModelImageAdmin(admin.ModelAdmin):
    ...
    def delete_model(self, request, obj):
        ...
        try:
            if obj.image_path:
                # 获取完整文件系统路径
                file_path = obj.image_path.path
                if os.path.exists(file_path):
                    try:
                        os.remove(file_path)
                    except Exception as e:
                        self.message_user(request, f'删除图片文件失败: {str(e)}', level='ERROR')
                else:
                    self.message_user(request, '图片文件不存在', level='WARNING')
        except Exception as e:
            self.message_user(request, f'获取图片路径失败: {str(e)}', level='ERROR')
        finally:
            # 确保总是调用父类方法删除数据库记录
            super().delete_model(request, obj)
                ...
                if obj.image_path:
                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            self.message_user(request, f'删除图片文件{file_path}失败: {str(e)}', level='ERROR')
            except Exception as e:
                self.message_user(request, f'处理图片{obj.pk}时出错: {str(e)}', level='ERROR')
        
        # 调用父类方法批量删除数据库记录
        super().delete_queryset(request, queryset)

    # 列表页配置
    list_display = ('id','model_serial', 'image_preview', 'image_state')
