...
def frontrender(request):
    if request.method == 'GET':
        # 获取所有 ModelStyle 信息
        model_styles = ModelStyle.objects.all().values('style_serial', 'style_name', 'style_dsc')
        
        # 获取所有审核通过的 Model 信息，并包含关联的 style_serial
        ...
        # 按 style_serial 分组 models 信息
        grouped_models = {}
        for model in models:
            style_serial = model['style_serial']
            if style_serial not in grouped_models:
                grouped_models[style_serial] = []
            # 获取审核通过的图片数量
            approved_images_count = ModelImage.objects.filter(
            ...
            # 将图片数量和图片路径存入 model 字典
            model['approved_images_count'] = approved_images_count
            model['approved_images'] = list(approved_images)
            grouped_models[style_serial].append(model)
        
        # 将分组后的 models 信息放入一个列表中，并与 ModelStyle 信息合并
        models_list = []
        for style in model_styles:
            style_serial = style['style_serial']
            models_for_style = grouped_models.get(style_serial, [])
            models_list.append({
                'style_serial': style_serial,
                'style_name': style['style_name'],
                'style_dsc': style['style_dsc'],
                'models': models_for_style
            })
...
class image_edit(APIView):
    """
        编辑图片接口
        输入: model_id, image_id, description
        输出: 编辑成功返回状态码，编辑失败返回错误信息
    """
    def get(self, request, model_id):
        try:
    ...
    def post(self, request, model_id):
        ...
        try:
            ...
            # 更新状态
            image_instance.image_state = '审核中'
            image_instance.save()
