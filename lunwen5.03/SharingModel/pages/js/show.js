...
function loadTexture(url) {
    loaders.tga.load(url, tga => {
        const texture = new THREE.Texture(tga);
        ...
        mesh.material.emissiveIntensity = 0.5; // 自发光强度，可以根据需要调整
    }, undefined, error => {
...
function loadObjWithMaterial(url, scale) {
    ...
    return new Promise((resolve, reject) => {
        loaders.mtl.load(mtlUrl, mtl => {
            loaders.mtl.setTexturePath(baseUrl);
            mtl.preload();
    ...
    $.ajax({
        ...
        success: async function(data) {
            try {
                ...
                const model_save_path = modelPath.split('\\media\\')[1].replace('\\', '/');// 处理路径
